...
func TestBuildSummary(t *testing.T) {
	tests := []struct {
		...
		branch   string
		err      error
		expected circleci.BuildSummaryResponse
	}{
		{
			"MultipleWorkflows",
			&circleci.Project{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			bsr, err := client.BuildSummary(ctx, tt.branch)
			...
			if len(bsr) != len(tt.expected) {
				t.Errorf("got %d, wanted %d", len(bsr), len(tt.expected))
			}
			...
			for i, got := range bsr {
				...
				if got.BuildNum != want.BuildNum {
					t.Errorf("BuildNum: got %d, want %d", got.BuildNum, want.BuildNum)
				}
				...
				if got.Workflows.Name != want.Workflows.Name {
					t.Errorf("Workflows.Name: got %q, want %q", got.Workflows.Name, want.Workflows.Name)
				}

				if got.Workflows.JobName != want.Workflows.JobName {
					t.Errorf("Workflows.JobName: got %q, want %q", got.Workflows.JobName, want.Workflows.JobName)
				}
			}
		})
	}
